...
func TestGetListRouter(t *testing.T) {
...

	{
		...
		for _, b := range list {
			...
			assert.IsType(t, news_model.NewsType_Announcement, b.Type)
			assert.IsType(t, []string{""}, b.Tags)
			assert.IsType(t, news_model.NewsStatusActive, b.Status)
