...
func TestGetList(t *testing.T) {
	...
	// 获取列表
	{
		...
		reports := make([]report_schema.Report, 0)
		...
		for _, b := range reports {
			...
			assert.IsType(t, report_model.ReportTypeBug, b.Type)
			assert.IsType(t, report_model.ReportStatusPending, b.Status)
...
func TestGetListByAdmin(t *testing.T) {
	...
	// 获取列表
	{
		r := report.GetListByAdmin(schema.Context{Uid: adminInfo.Id}, report.QueryAdmin{})
