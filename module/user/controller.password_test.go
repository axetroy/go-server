...
func TestUpdatePassword(t *testing.T) {
	...
	{
		// 2. 更新测试账号的密码, 旧密码错误
		r := user.UpdatePassword(schema.Context{Uid: userInfo.Id}, user.UpdatePasswordParams{
			OldPassword: "321321",
			NewPassword: "aaa",
		})
...
func TestUpdatePasswordByAdmin(t *testing.T) {
...

	{
		var newPassword = "321321"
		// 2. 更新测试账号的密码
		r := user.UpdatePasswordByAdmin(schema.Context{Uid: adminInfo.Id}, userInfo.Id, user.UpdatePasswordByAdminParams{
			NewPassword: newPassword,
		})
...
func TestUpdatePasswordRouter(t *testing.T) {
	...
	// 修改密码
	{
		...
		// 验证密码是否已修改
		user := user_model.User{Id: userInfo.Id}
