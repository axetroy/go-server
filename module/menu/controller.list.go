...
func findChild(allMenus []menu_schema.Menu, parentId string, myAccession []string, isSuperAdmin bool) (children []menu_schema.Menu) {
...
// 把子集的菜单挂载到父级菜单上
func set(menus []menu_schema.Menu, allMenus []menu_schema.Menu, myAccession []string, isSuperAdmin bool) (result []menu_schema.Menu) {
	for _, v := range menus {
...
func transform(menus []menu_schema.Menu, myAccession []string, isSuperAdmin bool) (data []menu_schema.Menu) {
	// 提取一级目录
	for _, v := range menus {
		// 如果是子菜单, 则跳过这个
		if v.ParentId != "" {
...
func GetList(context schema.Context, input Query) (res schema.Response) {
	var (
		err  error
		data = make([]menu_schema.Menu, 0)
	)
