...
func TestGetWallet(t *testing.T) {
	...
	{
		// 2. 获取获取钱包详情
		r := wallet.GetWallet(schema.Context{
			Uid: userInfo.Id,
		}, wallet_model.WalletCNY)

		assert.Equal(t, schema.StatusSuccess, r.Status)
		assert.Equal(t, "", r.Message)

		walletInfo := wallet_schema.Wallet{}

		assert.Nil(t, tester.Decode(r.Data, &walletInfo))

		assert.Equal(t, userInfo.Id, walletInfo.Id)
		assert.Equal(t, wallet_model.WalletCNY, walletInfo.Currency)
...
func TestGetWalletRouter(t *testing.T) {
	...
	// 获取详情
	{
		...
		n := wallet_schema.Wallet{}

		assert.Nil(t, tester.Decode(res.Data, &n))

		assert.Equal(t, userInfo.Id, n.Id)
		assert.Equal(t, wallet_model.WalletCNY, n.Currency)
