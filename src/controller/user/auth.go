...
func SendAuthPhone(c controller.Context) (res schema.Response) {
	var (
		err  error
		data schema.Profile
		tx   *gorm.DB
	...
	if err = telephone.Send(*userInfo.Phone, telephone.TemplateAuth, activationCode); err != nil {
		// 如果发送失败，则删除
...
func SendAuthEmailRouter(c *gin.Context) {
	...
	res = SendAuthEmail(controller.NewContext(c))
}
...
func SendAuthPhoneRouter(c *gin.Context) {
	...
	res = SendAuthPhone(controller.NewContext(c))
}
